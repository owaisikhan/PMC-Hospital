...
export default async function PatientsPage({
  ...
  const page = Math.max(
    1,
    Math.min(9999, Number.parseInt(rawPage ?? "1", 10) || 1),
  );
  const query = q.trim();
              ...
              <>
                <ul className="flex flex-col gap-2">
                  {patients.map((patient) => {
                    ...
                    const runningTotal = stay
                      ? (totalsByStay.get(stay.id) ?? 0)
                      : 0;
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <p className="text-base text-muted-foreground">
                    {total} {total === 1 ? "patient" : "patients"} · page {page}{" "}
                    of {lastPage}
                  </p>
                  <Pager
                    page={page}
  ...
  return (
    <div className="flex gap-2">
      {page > 1 ? (
        ...
        <span className={`${base} text-muted-foreground opacity-50`}>
          Previous
        </span>
      )}
      {page < lastPage ? (
