...
const TABS = ["orders", "tests"] as const
type LabTab = (typeof TABS)[number]
const TAB_LABELS: Record<LabTab, string> = { orders: "Orders", tests: "Tests & labs" }
const TAB_ICONS: Record<LabTab, TabIconName> = { orders: "flask", tests: "shield" }
...
export default async function LaboratoryPage({
  ...
  const visibleTabs: readonly LabTab[] = isAdmin ? TABS : (["orders"] as const)
  ...
  const [ordersResult, testsResult, patientsResult, settingsResult, labsResult, allTestsResult] =
    await Promise.all([
      tab === "orders" ? request : Promise.resolve({ data: null, error: null }),
      // lab_tests directly, not the view: a real FK embed of labs(name) works
      // here because this is the base table, and the order dialog only needs
      // the charge price anyway - cost never reaches this query.
      tab === "orders"
        ? supabase
            .from("lab_tests")
            .select("id, name, charge_price, labs(name)")
            .eq("is_active", true)
            .order("name")
      ...
      tab === "tests" ? supabase.from("labs").select("id, name, is_active").order("name") : Promise.resolve({ data: null }),
      tab === "tests"
        ? supabase.from("lab_tests_view").select("id, name, lab_id, charge_price, cost_price, is_active").order("name")
        : Promise.resolve({ data: null }),
    ])
  ...
  const tests: TestOption[] = (
    (testsResult.data ?? []) as unknown as {
      ...
      name: string
      charge_price: number
      labs: { name: string }[] | { name: string } | null
    }[]
  ).map((t) => ({
    id: t.id,
    name: t.name,
  ...
  const labs: LabRecord[] = ((labsResult.data ?? []) as { id: string; name: string; is_active: boolean }[]).map(
    (l) => ({ id: l.id, name: l.name, isActive: l.is_active })
  )
  const managedTests: LabTestRecord[] = (
    (allTestsResult.data ?? []) as {
      id: string
      name: string
      lab_id: string | null
      charge_price: number
    ...
    <>
      ...
      <div className="px-4 py-6 sm:px-6">
        <TabPanel panelKey={tab} index={visibleTabs.indexOf(tab)}>
          {tab === "tests" ? (
            <LabsSection labs={labs} tests={managedTests} />
          ) : (
