...
/** The hospital's logo mark and name, linking home. */
  ...
  return (
    <Link href="/" className="flex h-14 items-center gap-2 px-4">
      ...
      <span className="flex flex-col leading-tight">
        <span className="text-sm font-semibold tracking-tight text-sidebar-foreground">PMC</span>
        <span className="text-[0.6875rem] text-muted-foreground">Paeds Medical Complex</span>
      </span>
    </Link>
  )
}
...
/**
 * The navigation list - one copy, drawn by both the desktop sidebar and the
 * phone drawer (mobile-nav.tsx), so the two can never list different pages.
 *
 ...
 * crossing over.
 */
export function SidebarNav({ role, large = false }: { role: UserRole; large?: boolean }) {
...
/** The desktop sidebar. Below lg it is hidden and MobileNav takes over. */
